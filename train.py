...
tf.flags.DEFINE_string("input_data_file", "AAPL.xlsx", "Data source for the input headlines data")
tf.flags.DEFINE_string("output_data_file", "AAPL_min.xlsx", "Data source for the sentiment of the headlines")
...
# Load data
print("Loading data...")
x_text, y = data_helpers.get_data(FLAGS.input_data_file, FLAGS.output_data_file, True)
print(len(x_text))
# Build vocabulary
word_vec = data_helpers.load_glove(FLAGS.embedding_dim)
vocab, word2vec = data_helpers.process_vocab(FLAGS.embedding_dim,word_vec)
x, max_doc_len = [],0
for sentences in x_text:
	...
	for words in sentences.split():
		try:
			...
			word2vec = data_helpers.create_vector(words,FLAGS.embedding_dim,word_vec)
			vocab, word2vec = data_helpers.process_vocab(FLAGS.embedding_dim,word_vec)
			temp.append(vocab[words])
	x.append(temp)
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        cnn = TextCNN(
            ...
            word2vec = np.float32(np.array(word2vec)) ,
